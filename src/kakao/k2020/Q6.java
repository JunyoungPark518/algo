...
public class Q6 {
    public static int solution(int n, int[] weak, int[] dist) {
        ...
        for(int i=0; i<weak.length; i++) {
            weakMap.put(weak[i], 1);
        }

        AtomicInteger pos = new AtomicInteger();
        for(int i=0; i< dist.length; i++) {
            if(weakMap.keySet().contains(dist[i])) {
                weakMap.put(dist[i], 0);
                pos.set(dist[i]);
                dist[i] = 0;
            }
            else if(weakMap.keySet().contains(n - dist[i])) {
                weakMap.put((n-dist[i]), 0);
                dist[i] = 0;
            }
            else {
                for(int j=0; j<dist.length; j++) {
                    if(dist[j] == 0) {
                        AtomicInteger ji = new AtomicInteger(j);
                        weakMap.forEach((key, val) -> {
                            if(Math.abs(key - dist[ji.get()] - pos.get()) == 0) {
                                dist[ji.get()] = 0;
                            }
                        });
                    }
